...
namespace QuizGame {
	public class Grade2HistoryEasy :  MonoBehaviour {
		...
		[Header("Questions")]
		public List<QuestionSO> questionsGrade2HistoryEasy;
		...
		private void PickQuestion() {
			...
			int pick = Random.Range(0, questionsGrade2HistoryEasy.Count);
			// Pick it from the list
			ActiveQuestion = questionsGrade2HistoryEasy[pick];

			// Remove it so it won't get picked again during this game.
			questionsGrade2HistoryEasy.RemoveAt(pick);
		...
		private void False() {
			NextQuestionButton.gameObject.SetActive(true);
			EndGameButton.gameObject.SetActive(false);
			CorrectAnswerText.text = "<color=red>Wrong!</color>";
			ShowTrivia();
		...
		public void Next() {
			...
			if (questionsGrade2HistoryEasy.Count > 0 && questionsGrade2HistoryEasy.Count > 5) {
				PickQuestion ();
			} else {
				GameOver ();
			}
